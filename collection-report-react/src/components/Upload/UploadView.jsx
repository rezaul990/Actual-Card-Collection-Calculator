...
function UploadView({ uploadStep, onFileUpload, onOverdueFileUpload, onSkipOverdue, error, overdueError, mainFileName }) {
  ...
  // --- Step 2: Overdue file handlers ---
  const handleOverdueDragOver = (e) => {
    e.preventDefault()
    overdueDropZoneRef.current?.classList.add('dragover')
  }
  const handleOverdueDragLeave = () => {
    overdueDropZoneRef.current?.classList.remove('dragover')
  }
  const handleOverdueDrop = (e) => {
    e.preventDefault()
    overdueDropZoneRef.current?.classList.remove('dragover')
    const file = e.dataTransfer.files[0]
    if (file) onOverdueFileUpload(file)
  }
  const handleOverdueFileSelect = (e) => {
    const file = e.target.files[0]
    if (file) onOverdueFileUpload(file)
  }

  // --- Step 1: Upload main collection report ---
  if (uploadStep === 'main') {
    return (
  ...
  if (uploadStep === 'overdue') {
    return (
      <div className="upload-page">
        ...
        <div
          ref={overdueDropZoneRef}
          className="drop-zone"
          onDragOver={handleOverdueDragOver}
          onDragLeave={handleOverdueDragLeave}
          onDrop={handleOverdueDrop}
        >
          <div className="drop-zone-content">
            <div className="drop-zone-box" onClick={() => overdueFileInputRef.current?.click()}>
              ...
              <input
                ref={overdueFileInputRef}
                type="file"
                accept=".xls,.xlsx"
                onChange={handleOverdueFileSelect}
                style={{ display: 'none' }}
              />
            </div>
