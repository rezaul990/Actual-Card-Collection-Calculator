...
function AllAccountTable({ data, title, overdueData }) {
      ...
      <table className="report-table">
        ...
        <tbody>
          {accounts.map((acc, idx) => {
            const overdue = hasOverdue && acc.invoiceNo
            ...
            return (
              <tr key={idx}>
                ...
                {hasOverdue && (
                  <td style={{ textAlign: 'right' }}>{overdue > 0 ? overdue.toFixed(2) : '-'}</td>
                )}
              </tr>
            )
          })}
          ...
          <tr className="total-row">
            <td colSpan={hasOverdue ? 12 : 12} style={{ textAlign: 'right', fontWeight: 700 }}>
